...
(() => {
  ...
  function renderPublic(messages) {
    replaceMessages(messages.length ? messages.map((item) => `
      <article class="community-message-row tier-${escapeHtml(item.pointTier || "standard")} frame-${escapeHtml(item.betaFrame || "none")}${item.accountId === state.profile.accountId ? " mine" : ""}">
