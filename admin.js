...
(() => {
  ...
  $("community-warning-form").onsubmit = async (event) => { event.preventDefault(); const form = event.currentTarget; try { await request("/api/admin/community/warn", "POST", {username: $("community-warning-user").value, reason: $("community-warning-reason").value}); form.reset(); flash("admin-global-message", "Chat warning recorded.", "success"); } catch (error) { flash("admin-global-message", error.message, "error"); } };
  $("community-chat-action-form").onsubmit = async (event) => { event.preventDefault(); const form = event.currentTarget; try {
    ...
    form.reset(); flash("admin-global-message", "Chat access updated and audited.", "success");
  } catch (error) { flash("admin-global-message", error.message, "error"); } };
  $("announcement-form").onsubmit = async (event) => { event.preventDefault(); const form = event.currentTarget;
    ...
    try { const result = await request("/api/admin/announcements", "POST", {audience: $("announcement-audience").value,
      ...
      form.reset(); flash("announcement-message-result", `Announcement sent to ${result.recipientCount} account(s).`, "success"); await refreshQueues();
